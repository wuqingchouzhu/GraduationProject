    ...
    <el-container v-else style="height: 100vh;">
      ...
      <el-container>
        ...
        <el-main>
          ...
          <el-table :data="productList" border style="width: 100%" v-loading="loading" :row-class-name="tableRowClassName">
            ...
            <el-table-column prop="alertStatus" label="预警状态" width="100">
              <template #default="scope">
                <el-tag 
                  :type="scope.row.alertStatus === 1 ? 'danger' : 'success'"
                  size="small"
                >
                  {{ scope.row.alertStatus === 1 ? '预警中' : '正常' }}
                </el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="stockLevel" label="当前库存">
              <template #default="scope">
                <div style="display: flex; align-items: center; gap: 8px;">
                  ...
                  <span v-if="scope.row.reorderPoint && scope.row.reorderPoint > 0" style="font-size: 12px; color: #909399;">
                    预警: {{ scope.row.reorderPoint }}
                  </span>
                </div>
              </template>
            </el-table-column>
...
const tableRowClassName = ({ row }) => {
  return row.alertStatus === 1 ? 'warning-row' : ''
}

// 库存标签类型判断
const getStockTagType = (product) => {
  if (product.alertStatus === 1) return 'danger'
  if (product.stockLevel === 0) return 'info'
  if (product.stockLevel < 20) return 'warning'
  return 'success'
}
</script>
